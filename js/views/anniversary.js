...
// ---------- 人物卡片（精致字母头像 + 标签芯片云） ----------
function monogramAvatar(p) {
  const name = (p && p.name) || '?';
  const letter = [...name][0] || '?';
  const g = p && p.gender;
  const cls = g === '女' ? ' f' : g === '男' ? ' m' : '';
  return `<div class="pc-avatar mono${cls}" aria-hidden="true"><span class="pc-mono-ring"></span><span class="pc-mono-glow"></span><span class="pc-mono-letter">${esc(letter)}</span></div>`;
}
function personCard(p, i) {
  ...
  return `<div class="person-card" data-pid="${p.id}" style="--d:${((i || 0) * 0.06).toFixed(2)}s">
    ...
    <div class="pc-top">
      <div class="pc-avatar-wrap">${monogramAvatar(p)}</div>
      <div class="pc-info">
...
function personForm(rec = null) {
  formModal({
    ...
    fields: [
      ...
      { key: 'age', label: '年龄（选填）', type: 'number', value: rec?.age ?? '', min: 1, max: 120, placeholder: '如：24' },
      { key: 'occupation', label: '职业（选填）', span: 'full', value: rec?.occupation || '', placeholder: '如：博士在读 / 设计师 / 教师' },
      { type: 'section', label: '偏好标签' },
      { key: 'interests', label: '兴趣偏好标签', type: 'tagpick', span: 'full', value: rec?.interests || [], suggest: INTEREST_TAGS, hint: '点击「推荐标签」添加，也可自定义，数量不限（越准推荐越对）' },
      { key: 'style', label: '风格取向', type: 'tagpick', span: 'full', value: rec?.style || [], suggest: STYLE_TAGS, hint: '点击「推荐标签」添加，也可自定义，数量不限（越细定位越准）' },
      { type: 'section', label: '禁忌与预算' },
