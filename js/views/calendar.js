...
// 有截止时间的看板任务 = 日历上要出现的事项（kind:'reminder' 的迁移项也会带上 due）
function dueTasks() {
  return S().board.tasks.filter(t => t && t.due);
}
export function occurrencesOn(date, opt = {}) {
  const out = [];
  const d0 = startOfDay(date);
  dueTasks().forEach(t => {
    if (opt.tag && opt.tag !== 'all' && t.tag !== opt.tag) return;
    if (t.repeat && t.repeat !== 'none') {
      const occ = nextOccurrence(t, d0);
      if (isSameDay(occ, date)) out.push({ ...t, at: new Date(t.due), occAt: occ });
    } else if (isSameDay(t.due, date)) out.push({ ...t, at: new Date(t.due), occAt: new Date(t.due) });
  });
...
export function upcoming(days = 7, includeDone = false) {
  ...
  dueTasks().forEach(t => {
    ...
    const at = nextOccurrence(t, startOfDay(N));
    const dd = diffDays(at, N);
    if (dd >= -1 && dd <= days) out.push({ ...t, occAt: at, at: new Date(t.due) });
  });
  return out.sort((a, b) => a.occAt - b.occAt);
}
export function longTerm(minDays = 7) {
  const N = new Date();
  return dueTasks().filter(t => !t.done)
    .map(t => ({ ...t, occAt: nextOccurrence(t, startOfDay(N)), at: new Date(t.due) }))
    .filter(t => diffDays(t.occAt, N) > minDays)
    .sort((a, b) => a.occAt - b.occAt);
}
export function overdue() {
  const N = new Date();
  return dueTasks().filter(t => !t.done && (!t.repeat || t.repeat === 'none') && new Date(t.due) < N)
    .map(t => ({ ...t, occAt: new Date(t.due), at: new Date(t.due) })).sort((a, b) => a.occAt - b.occAt);
}
...
export function openReminderForm(rec = null, preset = {}) {
  ...
  formModal({
    ...
    fields: [
      ...
      { key: 'at', label: '时间（可精确到分，无年份上限）', type: 'datetime-local', required: true, value: rec ? toLocalInput(rec.due || rec.at) : (preset.at ? toLocalInput(preset.at) : toLocalInput(new Date(Date.now() + 3600000))) },
      ...
      { key: 'col', label: '看板分区', type: 'select', value: rec?.col || preset.col || 'personal', options: colOrderOpts(S().board) },
      { key: 'note', label: '备注', type: 'textarea', span: 'full', value: rec?.note || '', placeholder: '补充说明、链接等' }
    ],
    submitText: isEdit ? '保存修改' : '创建',
    extra: '<p class="small muted" style="margin-top:6px">提醒 = 看板任务：保存后会自动出现在「事项看板」对应分区，日历/待办同步显示。</p>',
    onSubmit: d => {
      const due = new Date(d.at).toISOString();
      const data = { title: d.title, due, advance: Number(d.advance), repeat: d.repeat, tag: d.tag, level: d.level, col: d.col, note: d.note };
      if (isEdit) store.patch('board.tasks', rec.id, data);
      else store.add('board.tasks', { ...data, kind: 'reminder', done: false, order: Date.now() });
      toast(isEdit ? '已更新（看板同步）' : '提醒已创建，并已加入看板', 'ok');
      requestPermission();
      rerender();
    }
  });
...
function drawSide() {
  ...
  const done = S().board.tasks.filter(t => t.done).sort((a, b) => (b.doneAt || 0) - (a.doneAt || 0)).slice(0, 12);
  $('#sideLists').innerHTML = `
    ${listCard('⏳ 近期待办（7 天内）', up.filter(x => !x.done), true)}
    ${listCard('🎯 远期重要事项', lt.slice(0, 20), true)}
    ${od.length ? listCard('⚠️ 已逾期', od, true) : ''}
    ${listCard('✅ 已完成事项', done.map(r => ({ ...r, occAt: new Date(r.at || r.due) })), false)}
  `;
  $('#sideLists').onclick = e => {
...
export function openDetail(id) {
  const r = S().board.tasks.find(x => x.id === id); if (!r) return;
  ...
  modal({
    ...
    onOpen: (b, close) => {
      document.getElementById('delRem').onclick = () => { close(); confirmDlg('确定删除该提醒？', () => { store.remove('board.tasks', id); toast('已删除'); rerender(); }); };
...
// 供其他模块创建提醒（写入看板任务，带 kind:'reminder' 标记）
export function quickCreate(title, at, tag = 'research', extra = {}) {
  return store.add('board.tasks', {
    kind: 'reminder', title, due: new Date(at).toISOString(), tag, level: extra.level || 'mid',
    repeat: extra.repeat || 'none', advance: extra.advance ?? 1440, note: extra.note || '', done: false, col: 'personal', order: Date.now()
  });
}
